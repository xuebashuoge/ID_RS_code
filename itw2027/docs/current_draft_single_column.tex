...
We consider three types of boolean functions: identification ($S=1$), rank test $f(i)=\1\{int(i)\leq 20\}$ ($S=21$), and the exact-threshold $f(i)=\1\{\|i\|_1=2\}$ ($S=\binom{m}{2}$).
The design error exponent is $E_2=0.1$. We choose even $n_t$, and set $r=n_t/2$, $T=2^r$, and $m=rK$ where $K$ is solved by $\max\{K\in\N:KS(rK)\leq 2^{n_t(1/2-E_2)}\}$. Therefore, the finite-length rate is calculated by $R=\log(m)/n_t$.
...
In the noisy channel setting, we transmit BPSK symbols over an AWGN channel and utilize a DVB-S2 LDPC code with $N_b=64\,800$, belief-propagation decoding, and at most 50 iterations. Each SNR point contains $2{,}500$ frames.

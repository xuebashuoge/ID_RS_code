...
Use two wide figures with grouped small panels and one small three-row table, with about 250–350 words of discussion. The revised figures are approximately 7.1 inches wide and 4.0/4.15 inches high before caption placement. This takes more vertical space than the old overlays; recheck the full manuscript page budget rather than assuming the earlier one-page estimate. If space is tighter, use only the error panels in the paper and move the rate-sequence panel to the linked preprint; keep the finite-rate table. Do not add a separate third rate/latency figure.
...
**Panel (a).** Three stacked panels, one per function, with common probability scales. Horizontal axis: tag length `n_t=24,26,...,40`. Vertical axis: exact per-message FP probability on a log scale. Filled circles and solid lines show medians; hollow squares show sample maxima; gray dashed lines show `B=S(K−1)/T`. Where values coincide, a small filled dot sits inside the larger hollow square and the dashed reference remains visible. There is no shading, displacement, or artificial separation of numerical values. Fifth–95th percentiles remain in the saved table. The sample comprises 2,000 messages per setting except ID at n_t=38 and 40, with 200 each. Every message is evaluated at all T indices. Legends identify quantities, while row labels identify function families.
...
**Panel (b).** Horizontal axis: tag length 24–4096, logarithmic scale. Vertical axis: finite exponential rate `R_t=log2(m)/n_t`. Same three colors. Solid: E=0.1; dashed: E(n_t)=1/sqrt(n_t). Horizontal dotted references: 1/2 and 1/6. Separate legends identify the exponent sequences, asymptotic references, and function families. Sparse circle/square/triangle markers at original coordinates distinguish the families as curves approach one another.
...
**Panel (b).** Right column with the same three function rows and common axes. Observed conditional FP uses colored hollow circles without a connecting line. Paired noiseless FP uses a thin solid reference line, and `min(1,B+observed payload FER)` uses a gray dashed line explicitly labelled as a diagnostic. Points on the reference show agreement without overprinting a second coincident curve. No shading is drawn. This shows the intrinsic collision floor and separate channel contribution; channel errors can also remove an intrinsic false positive, so noisy FP need not exceed noiseless FP at every SNR.

function [llr, channel_state] = transmit_bpsk_llr(bits, channel_type, ebno_db, payload_rate)
%TRANSMIT_BPSK_LLR BPSK channel model with correctly scaled soft output.
%
% Bit zero maps to +1. LLR is log(P(bit=0)/P(bit=1)).

    validateattributes(payload_rate, {'numeric'}, ...
        {'scalar', 'positive', '<=', 1});
    gamma_b = 10^(double(ebno_db)/10);
    N0 = 1 / (double(payload_rate) * gamma_b);
    tx = 1 - 2*single(bits);
    ...
    switch lower(channel_type)
        case 'awgn'
            ...
            rx = tx + noise;
            llr = single(4/N0) .* rx;
            channel_state = struct('N0', N0, ...
                'payload_rate', payload_rate, ...
                'effective_rate', payload_rate);

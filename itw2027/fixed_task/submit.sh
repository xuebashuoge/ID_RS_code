...
banks=$(sbatch --parsable --dependency="afterok:${smoke%%;*}" --array="0-$(count "$FT_MANIFEST")%4" --time=03:00:00 --mem=32G --export=ALL itw2027/fixed_task/job.slurm)
printf 'banks=%s\n' "$banks" | tee -a "$FT_OUT/jobs.txt"
export FT_MANIFEST="$FT_OUT/noisy.json"
noisy=$(sbatch --parsable --dependency="afterok:${banks%%;*}" --array="0-$(count "$FT_MANIFEST")%8" --time="$channel_time" --export=ALL itw2027/fixed_task/job.slurm)
printf 'noisy=%s\n' "$noisy" | tee -a "$FT_OUT/jobs.txt"
export FT_MANIFEST="$FT_OUT/noiseless.json"
noiseless=$(sbatch --parsable --dependency="afterok:${smoke%%;*}" --array="0-$(count "$FT_MANIFEST")%2" --time="$noiseless_time" --mem=32G --export=ALL itw2027/fixed_task/job.slurm)
